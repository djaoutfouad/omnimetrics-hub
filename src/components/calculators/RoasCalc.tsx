...
export const RoasCalc: React.FC<Props> = ({ currency }) => {
  ...
  const netAdProfit = revenue - adSpend;
  const cpa = conversions > 0 ? adSpend / conversions : 0;
  const netTrueProfit = revenue - adSpend - cogs;
  const trueRoi = (adSpend + cogs) > 0 ? (netTrueProfit / (adSpend + cogs)) * 100 : 0;

  const handleCopy = () => {
    const text = `ROAS & Ad Spend Summary:\n- Total Ad Spend: ${currency}${adSpend.toLocaleString()}\n- Revenue Generated: ${currency}${revenue.toLocaleString()}\n- ROAS: ${roasMultiplier.toFixed(2)}x (${roasPercent.toFixed(0)}%)\n- Net Ad Profit: ${currency}${netAdProfit.toFixed(2)}\n- Cost Per Acquisition (CPA): ${currency}${cpa.toFixed(2)} (${conversions} conversions)\n- True Net Profit (after COGS): ${currency}${netTrueProfit.toFixed(2)} (ROI: ${trueRoi.toFixed(1)}%)`;
      ...
      <div className="p-4 bg-slate-900 text-white rounded-2xl shadow-inner space-y-3">
        <div className="flex justify-between items-baseline">
          <div>
            ...
            <div className="text-[11px] text-slate-400">Revenue per $1 spent</div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-slate-800 pt-2.5 text-xs">
          <div>
            ...
            <span className="font-bold text-emerald-400 text-sm">{currency}{netAdProfit.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          </div>
          <div>
