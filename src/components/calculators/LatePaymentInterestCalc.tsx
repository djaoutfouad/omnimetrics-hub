...
export const LatePaymentInterestCalc: React.FC<Props> = ({ currency }) => {
  ...
  // Calculations
  const dailyInterest = (principal * (annualRate / 100)) / 365;
  const totalInterest = dailyInterest * daysOverdue;
  const totalDue = principal + totalInterest + statutoryFee;

  const handleCopy = () => {
    const text = `Invoice Late Payment & Interest Summary:\n- Principal Invoice Amount: ${currency}${principal.toLocaleString('en-US', { minimumFractionDigits: 2 })}\n- Days Overdue: ${daysOverdue} days\n- Annual Statutory/Contract Rate: ${annualRate}%\n- Accrued Late Interest: ${currency}${totalInterest.toFixed(2)} (${currency}${dailyInterest.toFixed(2)} / day)\n- Statutory Recovery / Admin Compensation: ${currency}${statutoryFee.toFixed(2)}\n- TOTAL OUTSTANDING DEBT DUE: ${currency}${totalDue.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
      ...
      <div className="p-4 bg-slate-900 text-white rounded-2xl shadow-inner space-y-3">
        <div className="flex justify-between items-baseline">
          <div>
            ...
            <div className="text-[11px] text-slate-400">Principal + {daysOverdue} days interest + recovery fee</div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-slate-800 pt-2.5 text-xs">
          <div>
            ...
            <span className="font-bold text-white text-sm">{currency}{principal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          </div>
          <div>
            <span className="text-slate-400 block text-[11px]">Recovery Compensation:</span>
            <span className="font-bold text-emerald-400 text-sm">{currency}{statutoryFee.toFixed(2)}</span>
          </div>
        </div>
      </div>
