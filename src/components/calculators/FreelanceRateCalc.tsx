...
export const FreelanceRateCalc: React.FC<Props> = ({ currency }) => {
  ...
  const totalGrossRequired = takeHome + taxesAndBenefits + expenses;
  const hourlyRate = billableHours > 0 ? totalGrossRequired / billableHours : 0;
  const dayRate = hourlyRate * 8;
  const weeklyRate = hourlyRate * (billableHours / 48); // 48 active working weeks
  const monthlyGross = totalGrossRequired / 12;

  const presets = [
    { label: 'Conservative (800 hrs)', hrs: 800 },
    { label: 'Standard (1,000 hrs)', hrs: 1000 },
    { label: 'High Capacity (1,250 hrs)', hrs: 1250 },
  ];

  const handleCopy = () => {
    const text = `Freelance Rate Pricing Formula:\n- Target Take-Home: ${currency}${takeHome.toLocaleString()}/yr\n- Taxes, Healthcare & Buffers: ${currency}${taxesAndBenefits.toLocaleString()}/yr\n- Operating Expenses: ${currency}${expenses.toLocaleString()}/yr\n- Total Gross Revenue Required: ${currency}${totalGrossRequired.toLocaleString()}/yr\n- Billable Capacity: ${billableHours} hrs/yr\n- Minimum Hourly Rate: ${currency}${hourlyRate.toFixed(2)} / hr\n- Day Rate (8 hrs): ${currency}${dayRate.toFixed(2)} / day`;
  ...
  return (
    <div className="space-y-5 text-slate-800">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-bold text-slate-700 block mb-1">
            Taxes & Health Insurance ({currency})
          </label>
          <div className="relative">
        ...
        <div>
          <label className="text-xs font-bold text-slate-700 block mb-1">
            Software & Business Expenses ({currency})
          </label>
          <div className="relative">
            <span className="absolute left-3 top-2 text-slate-400 font-bold text-xs">{currency}</span>
            <input
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="text-xs font-bold text-slate-700">Annual Billable Hours</label>
          <span className="text-[11px] text-slate-500 font-semibold">{billableHours} hrs/yr (~{(billableHours / 48).toFixed(1)} hrs/week)</span>
        </div>
        <input
          type="number"
          min="100"
          ...
          step="50"
          value={billableHours || ''}
          onChange={(e) => setBillableHours(parseFloat(e.target.value) || 1)}
        ...
        <div className="flex gap-1.5">
          {presets.map((p) => (
            <button
              ...
              className={`text-[10px] py-1 px-2 rounded-lg border font-medium transition ${
                billableHours === p.hrs
      ...
      <div className="p-4 bg-slate-900 text-white rounded-2xl shadow-inner space-y-3">
        ...
        <div className="flex justify-between items-baseline border-t border-slate-800 pt-2.5">
          <div>
            ...
            <div className="text-[11px] text-slate-400">Base quote baseline</div>
          </div>
